...
To fix convergence issues, it helps to center the preditors, increase the iterations and warmups, and set some priors. Please center Elevation, try iter=6000, warmup=4000, add some priors that seem reasonable, and re-fit the model. Please call the new model m.elev2, then use plot(), summary(), and pairs() on it.
...
Let's plot the posterior prediction on the measurement scale using the estimate_expectation() function from the modelbased package.
```{r}
...
Remember how we have a log link? The predict_response() function by default back-transforms from the log scale (log(Endemics)) of the linear model to the measurement scale of Endemics. To see the linear model, however, we can set predict = 'link'.
```{r}
