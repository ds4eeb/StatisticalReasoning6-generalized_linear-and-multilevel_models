...
It does not look normal - there is a big peak near zero, and the distribution is definitely not symmetrical. It also is never negative and does not cross zero.
...
Plot the model predicts, first on the log link scale, then on the response scale. Please show the data in your plot on the response scale.
...
This dataset has the number of male and female turtles hatched at different temperature. Let's calculate the total hatched and the proportion female.
```{r}
...
### Run model
Since these are proportion data, a binomial link is good. Let's fit a model:
```{r}
m.turt <- 
